...
Core email functionality for the Pop Framework. Send emails via SMTP with support for queuing, templates, attachments, and tracking.
...
```bash
# Process once
./pop email:queue:process

# Process with custom batch size
./pop email:queue:process --limit=50

# Run as daemon (continuous processing)
./pop email:queue:process --daemon

# Process one batch only
./pop email:queue:process --once
```
...
command=php /path/to/pop email:queue:process --daemon
autostart=true
autorestart=true
user=www-data
numprocs=2
...
```cron
* * * * * cd /path/to/app && ./pop email:queue:process --once
```
...
1. Check queue: `./pop email:queue:process --once`
2. Check logs: `tail -f storage/logs/email.log`
3. Test config: `./pop email:test your@email.com`
...
Part of Pop Framework - MIT License
